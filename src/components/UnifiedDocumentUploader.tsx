...
export const UnifiedDocumentUploader = ({ applicationId, onUpload, isUploading = false, uploadError = null }) => {
  ...
  return (
    <div className="w-full max-w-full overflow-hidden">
      <div {...getRootProps()} className={`border-2 border-dashed rounded-xl p-6 cursor-pointer mb-6 transition-all duration-300 ${isDragActive ? 'border-[#3BAA75] bg-[#3BAA75]/5 scale-[1.02]' : 'border-gray-300 hover:border-[#3BAA75] hover:bg-[#3BAA75]/5'}`}>
        <input {...getInputProps()} />
      ...
      {(uploadError || validationError) && (
        <div className="mb-6 p-4 bg-red-50 text-red-600 rounded-xl flex items-center gap-2 shadow-sm">
