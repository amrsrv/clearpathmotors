/*
  ...
  1. Schema Changes
    - Modify `notifications` table to allow NULL user_id
    - Modify `chats` table to allow NULL user_id
    - Add temp_user_id to notifications table
  
  2. Security
    - Update RLS policies for notifications to allow anonymous access
    - Update RLS policies for chats to allow anonymous access
    - Update RLS policies for applications to ensure proper access
*/
...
CREATE POLICY "Users can view own notifications" 
  ...
  USING (
    ...
    (temp_user_id IS NOT NULL AND auth.uid() IS NULL AND temp_user_id = current_setting('request.jwt.claims', true)::json->>'sub')
  );
...
CREATE POLICY "Users can read own chats" 
  ...
  USING (
    ...
    (anonymous_id IS NOT NULL AND auth.uid() IS NULL AND anonymous_id = current_setting('request.jwt.claims', true)::json->>'sub')
  );
...
CREATE POLICY "applications_select_policy" 
  ...
  USING (
    ...
    ((auth.uid() IS NOT NULL) AND (dealer_id = auth.uid()) AND EXISTS (
      SELECT 1 FROM users WHERE users.id = auth.uid() AND (users.raw_app_meta_data->>'role') = 'dealer'
    )) OR 
    ((auth.uid() IS NOT NULL) AND EXISTS (
      SELECT 1 FROM users WHERE users.id = auth.uid() AND (users.raw_app_meta_data->>'role') = 'super_admin'
    ))
  );
  ...
  USING (
    EXISTS (
      ...
      WHERE chats.id = chat_messages.chat_id AND (
        ...
        (chats.anonymous_id IS NOT NULL AND auth.uid() IS NULL AND chats.anonymous_id = current_setting('request.jwt.claims', true)::json->>'sub')
      )
    )
  );
...
-- Fix the Dashboard.tsx component to handle loading states better