/*
  # Fix for application flags and admin settings

  1. New Tables
    - Ensures application_flags and admin_settings tables exist
    - Adds proper constraints and relationships
  
  2. Security
    - Enables RLS on tables
    - Adds policies with existence checks to prevent errors
  
  3. Functions
    - Creates or replaces auto_flag_application function
    - Creates or replaces log_stage_change function
    - Creates or replaces update_application_status_on_stage function
    - Creates or replaces log_admin_message function
  
  4. Triggers
    - Updates triggers with proper existence checks
...
-- Enable RLS on application_flags if not already enabled
ALTER TABLE application_flags ENABLE ROW LEVEL SECURITY;

-- Enable RLS on admin_settings if not already enabled
ALTER TABLE admin_settings ENABLE ROW LEVEL SECURITY;

-- Create policies for application_flags with existence check
DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies 
    WHERE tablename = 'application_flags' AND policyname = 'Admins can manage flags'
  ) THEN
...
-- Create policies for admin_settings with existence check
DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies 
    WHERE tablename = 'admin_settings' AND policyname = 'Admins can manage settings'
  ) THEN
...
-- Create trigger for auto-flagging applications with existence check
...
-- Create trigger for application stage changes with existence check
...
-- Create trigger for updating application status based on stage with existence check
...
-- Create trigger for admin messages with existence check
