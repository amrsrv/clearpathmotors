/*
  ...
  1. Changes
    ...
    - Update RLS policies to remove dealer-specific access rules
  
  2. Security
    - Update RLS policies to maintain proper access control
    - Remove dealer-specific role checks
*/

-- First, remove the dealer_id column from applications table
ALTER TABLE applications DROP COLUMN IF EXISTS dealer_id;

-- Drop the applications_with_dealer view if it exists
...
-- Update user_role_enum to remove 'dealer' variant
-- Note: We can't directly modify enum types in PostgreSQL, so we need to create a new one
-- and update all columns that use it
