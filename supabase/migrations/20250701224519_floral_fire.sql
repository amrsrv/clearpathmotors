...
CREATE POLICY "applications_select_policy" 
  ...
  USING (
    ...
    ((auth.uid() IS NOT NULL) AND (dealer_id = auth.uid()) AND EXISTS (
      SELECT 1 FROM auth.users WHERE auth.users.id = auth.uid() AND ((auth.users.raw_app_meta_data->>'role')::text = 'dealer')
    )) OR 
    ((auth.uid() IS NOT NULL) AND EXISTS (
      SELECT 1 FROM auth.users WHERE auth.users.id = auth.uid() AND ((auth.users.raw_app_meta_data->>'role')::text = 'super_admin')
    ))
  );
