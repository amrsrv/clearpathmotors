/*
  ...
  1. New Tables
    - `applications`: Tracks loan application status and progress
    - `application_stages`: Records stage-specific information
    - `documents`: Manages user document uploads
    - `notifications`: Handles in-app notifications
...
BEGIN
  ...
  IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'idx_application_stages_application_id') THEN
